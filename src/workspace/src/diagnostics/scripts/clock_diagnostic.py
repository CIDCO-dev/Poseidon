...
# The "gps" module comes from python3-gps (gpsd)
from gps import gps, WATCH_ENABLE, WATCH_NEWSTYLE

class ClockDiagnostic(DiagnosticsTest):
    """
    Checks the latency between the system time and the GPS time provided by gpsd.
    - OK if |system - gps| <= 30 ms
    - WARN otherwise
    - ERROR if gpsd is missing or no TPV/time is received within 2s
    """
    ...
    def update(self):
        ...
        try:
            while (time.time() - start) < self.timeout_s and not rospy.is_shutdown():
                report = session.next()  # may raise StopIteration
                # Look for a TPV report with a "time" field
                if isinstance(report, dict):
                    cls = report.get("class")
                    ts_str = report.get("time")
                else:
                    # Some bindings expose fields as attributes
                    cls = getattr(report, "class", None)
                    ts_str = getattr(report, "time", None)

                if cls == "TPV" and ts_str:
                    # gpsd provides an ISO8601 timestamp, often ending with "Z"
                    if ts_str.endswith("Z"):
                        ts_str = ts_str[:-1]
                    try:
                        gps_epoch = datetime.fromisoformat(ts_str).timestamp()
                    except ValueError:
                        # Fallback format: "YYYY-mm-ddTHH:MM:SS" without microseconds
