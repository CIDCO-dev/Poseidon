...
#ifndef CARTESIANTOGEODETICFUKUSHIMA_H
#define CARTESIANTOGEODETICFUKUSHIMA_H
...
public:
    ...
    void ecefToLongitudeLatitudeElevation(Eigen::Vector3d & ecefPosition, PositionPacket & positionGeographic) {
        ...
        if (x == 0.0 && y == 0.0 && z == 0.0) {
            positionGeographic.latitude = 0.0;
            positionGeographic.longitude = 0.0;
            positionGeographic.altitude = 0.0;
            return;
        }
        ...
        if (x == 0.0 && y == 0.0 && z != 0.0) {
            if (z > 0) {
                positionGeographic.latitude = M_PI_2*R2D;
            } else {
                positionGeographic.latitude = -M_PI_2*R2D;
            }

            positionGeographic.longitude = 0.0;
            positionGeographic.altitude = std::abs(z) - b;
            return;
        }
        ...
        if (z == 0.0) {
            positionGeographic.latitude = 0.0;
            positionGeographic.longitude = estimateLongitude(x, y, p)*R2D;
            positionGeographic.altitude = std::sqrt(x * x + y * y) - a;
            return;
        }
        ...
        positionGeographic.latitude = lat*R2D;
        positionGeographic.longitude = lon*R2D;
        positionGeographic.altitude = h;
    }
