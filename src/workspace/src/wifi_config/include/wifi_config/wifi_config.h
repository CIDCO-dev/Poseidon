...
/*			if(wifiTransferActivated && !isWifiConfigured() ){*/
/*				set_wifi_config();*/
/*			}*/
		};
...
/*		}*/
		...
		void configurationCallBack(const setting_msg::Setting &setting){
			...
			if(setting.key == "wifiSSID"){
				if(setting.value != this->ssid){
					this->newSsid = setting.value;
				}
			}
			
			else if(setting.key == "wifiPassword"){
				if(setting.value != this->wifiPassword){
				...
				if(setting.value == "true" && !wifiTransferActivated){
					this->wifiTransferActivated = true;
				...
				else if(setting.value != "true" && wifiTransferActivated){
					this->wifiTransferActivated = false;
					if(is_connection_set()){
						deleteOldConnection();
					}
